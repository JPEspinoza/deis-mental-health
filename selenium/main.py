...
def get_list_items(button_id, list_id):
    ...
    # open dropdown
    WebDriverWait(driver, 30).until(expected_conditions.presence_of_element_located((By.ID, button_id))).click()
...
WebDriverWait(driver, 60).until(expected_conditions.frame_to_be_available_and_switch_to_it((By.ID,"VANextLogon_iframe")))
...
for region in get_list_items("__select0", "__list2"):
    WebDriverWait(driver, 10).until(expected_conditions.presence_of_element_located((By.ID, "__select0"))).click()
    WebDriverWait(driver, 10).until(expected_conditions.presence_of_element_located((By.CSS_SELECTOR, f"[title*=\"Región: {region}\"]"))).click()
    for servicio in get_list_items("__select1", "__list3"):
        WebDriverWait(driver, 10).until(expected_conditions.presence_of_element_located((By.ID, "__select1"))).click()
        WebDriverWait(driver, 10).until(expected_conditions.presence_of_element_located((By.CSS_SELECTOR, f"[title*=\"Servicio de Salud: {servicio}\"]"))).click()
        for comuna in get_list_items("__select2", "__list4"):
            comuna = comuna.replace("'", "\\'") # replace O'HIGGINS with O\'HIGGINS
            print(comuna)
            WebDriverWait(driver, 10).until(expected_conditions.presence_of_element_located((By.ID, "__select2"))).click()
            WebDriverWait(driver, 10).until(expected_conditions.presence_of_element_located((By.CSS_SELECTOR, f"[title*=\"Comuna: {comuna}\"]"))).click()
            for establecimiento in get_list_items("__select3", "__list5"):
                WebDriverWait(driver, 10).until(expected_conditions.presence_of_element_located((By.ID, "__select3"))).click()
                WebDriverWait(driver, 10).until(expected_conditions.presence_of_element_located((By.CSS_SELECTOR, f"[title*=\"Establecimiento: {establecimiento}\"]"))).click()
                sleep(2)
